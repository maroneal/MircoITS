		...
		default: // Father process - (TCP Server)
			...
			while (!end)
			{
                ...
                if (clientStatus > 0)    // If something happened on the socket
				{
                    ...
                    unserializeStructMessage(&mes, buffer, SIZE_BUF);   // Unserialiez the message to transform the string into a message structure

                    if(!strcmp(mes.wifiIp, IP_LOOPBACK))	// If a message comes from the python program
					{
						if(mes.command == COMMAND_NOTHING)	// If the message is the sent position
						{
							position.x = mes.position.x;	// Actualization of the position of the robot
                    ...
                    if(!strcmp(mes.wifiIp, IP_DIST))	// If the message comes from the other robot
                    {
				...
				if(getTime() >= timer + TIME_PERIOD_UPDATE)
				{
					...
					writePipe(fd1, "TIME", sizeof("TIME"));
					readPipe(fd2, buffer, SIZE_BUF);
